...
void xmpp_SHA1_Transform(uint32_t state[5], const uint8_t buffer[64]);
...
void xmpp_SHA1_Update(SHA1_CTX* context, const uint8_t* data, const size_t len)
{
    ...
    if ((j + len) > 63) {
        ...
        xmpp_SHA1_Transform(context->state, context->buffer);
        for ( ; i + 63 < len; i += 64) {
            xmpp_SHA1_Transform(context->state, data + i);
        }
        j = 0;
    }
    else i = 0;
...
void xmpp_SHA1_Final(SHA1_CTX* context, uint8_t digest[SHA1_DIGEST_SIZE])
{
    ...
    xmpp_SHA1_Update(context, finalcount, 8);  /* Should cause a xmpp_SHA1_Transform() */
...
FILE* file;
    ...
    fclose(file);
    for (i = 0; i < xmpp_SHA1_DIGEST_SIZE/4; i++) {
        for (j = 0; j < 4; j++) {
